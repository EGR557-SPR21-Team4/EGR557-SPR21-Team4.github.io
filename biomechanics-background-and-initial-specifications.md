...
After analyzing the American Cockroach via  research papers, the team found five highly cited sources [6-10] that use the organism to create a  biomechanical robot. After narrowing the results down to three papers [6, 7, 8],  the team studied and gathered the biomechanical data described in the section below.

 The first report conducted high-level experimentation of the cockroach, specific to movement, linearly, and turning. Furthermore, it drew direct parallels to mathematical expressions that in turn can be used and applied to hexapod robotics. The experiment captured many different variables: speed, linear acceleration, curvature, angle, and more. It related these findings to understand cockroaches' movement processes and aimed to answer their hypothesis, which was, "…[how] leg and body morphology and function affect potential turning performance." Ultimately, their research led to a significant amount of information being categorized and documented about the working relationship of legs, movement, and their ' overall mathematical relationship. The derived formulas and real-world data tables are why this paper is so valuable to our project [6]. 
  
  Additionally, the second paper aimed to produce a dynamic, lightweight, bio-inspired robot that focused on the cockroach's tripod gait. The kinematics of movement, designs, and robotic specifications are included, which will be of value, given the direction this project may take. Furthermore, designated schematics of movement mechanisms are included. This includes their schematic of four-bar  and slider-crank kinematics, accompanied by mathematical formulas useful in calculating the input from the slider-crank and the force output of the robot. The experimental research data is also included, such as robot speed, stride frequency and positional data, which will all help give a starting place for our data gathering phase, should this project become realized [7]. 
  
  The final report focuses on bio-inspired cockroach hexapods that can climb over obstacles over 230% the length of their leg. The document shows the design used to mimic the cockroach as well as the physical dimensions, variables, angles, and physics of their design. In addition to their design layout, the report shows the hexapods’ software diagram, explaining how the robot makes decisions, positions its body, and moves its legs. It also provides a list of sensors and actuators used to achieve this obstacle scaling design. The document is full of free body diagrams, including the physics equations used throughout. The hexapod was put through many tests using different obstacle heights and the results are recorded in the report. Alongside the testing results is a table of error between the kinematics simulations and actual results. This reference is beneficial to our team because it lists the physical dimensions used for their cockroach mimicking robot, explains the equations and kinematics used throughout the design process, and depicts the results of their simulations and tests [8]. 
